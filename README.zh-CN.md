...
- Windows 安装完成页默认勾选“运行 MD阅读助手”，点击“完成”即可直接启动，也可以取消勾选。
- Windows 安装向导支持简体中文与 English；真正的全新安装会在软件首次启动时选择一次界面语言，升级安装不会弹出首次语言选择。
...
- 编辑状态下使用 `Ctrl/Cmd + F` 会直接查找 Markdown 源码，高亮匹配项并滚动定位，不再跳回预览页。
