...
“关于轻阅 Markdown”提供“参与产品改进计划”勾选项。启用后，仅在软件发生异常时后台静默提交清理本地路径后的错误日志、服务器解析的国家/省/市、Windows／macOS／Linux 系统类型和软件版本。不会上传 IP、安装标识、详细系统版本、架构、使用行为、Markdown 内容、文件名或文件路径；断网、内网、超时和接口异常均静默忽略，不影响软件功能。取消勾选后不再提交错误日志。
