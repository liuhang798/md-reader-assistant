...
- 主题颜色与白天/黑夜模式拆分为两个独立选项，可自由组合并分别记忆。
- 新增清新绿、晴空蓝、活力橙、灵动紫、珊瑚红、湖水蓝、雾蓝灰和陶土棕 8 种强调色，默认使用清新绿。
- 更新应用品牌图标为透明圆角的亮绿色书页与羽毛标识；应用内 Logo 会跟随强调色切换，系统图标保持默认绿色。
- 自动迁移旧版本的完整主题设置，恢复为最接近的强调色与明暗模式组合。
...
推送 `v2.2.6` 等版本标签后，`.github/workflows/release.yml` 会自动构建三个系统的安装包并发布到 GitHub Releases。客户端会根据仓库的最新稳定 Release 提醒更新。
