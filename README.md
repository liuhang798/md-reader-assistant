...
- 新增图表、学科公式与 Markdown 格式三份内置完整案例，首页和文档库均可直接打开，且不会占用最近阅读记录。
- 重做首页快速上手区域，新增完整快捷键指南；阅读页可关闭当前预览并返回首页，同时保留最近阅读记录。
- 本地图片支持选择、拖放和剪贴板粘贴，自动归档到文档旁的 `assets` 目录；插入时可设置 10%–100% 显示宽度。
- Mermaid 与 ECharts 在暗色模式、主题切换、甘特图及树图／思维导图／桑基图等复杂图表中保持清晰，宽幅数据图表导出到 Word／HTML 时不再裁切或变形。
- 置顶文档与普通最近文档左侧对齐，悬停显示排序提示，按住整张卡片任意位置即可拖动排序，普通单击仍会打开文档。
- macOS 应用内更新改为校验并原子替换完整签名应用包，避免裸可执行文件更新破坏代码签名，并提供失败回滚保护。
