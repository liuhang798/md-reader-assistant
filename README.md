...
macOS 版本使用在轻薄标题栏中垂直居中的系统原生窗口控制按钮、适配明暗模式的顶部工具条与应用菜单；进入全屏后 Logo 和软件名称会自动向左对齐，退出后恢复窗口按钮安全间距。支持标准 Command 快捷键：`Command + W` 关闭窗口并留在后台，`Command + Q` 真正退出应用。全屏关闭时会先退出全屏再隐藏到后台；编辑器和资源浏览器采用延迟初始化，减少冷启动等待。
