...
- macOS 跟随系统明暗外观时允许临时手动切换，系统下次外观变化后自动恢复跟随。
- 修复 macOS 后台运行时点击 Dock 图标不显示窗口或未置于最前方的问题。
- 最近阅读新增右键菜单，支持直接编辑、打开所在文件夹和移除记录。
- 修复 macOS 全屏下点击关闭按钮偶发只退出全屏而不隐藏窗口的问题。
...
推送 `v2.3.1` 等版本标签后，`.github/workflows/release.yml` 会自动构建三个系统的安装包并发布到 GitHub Releases。客户端会根据仓库的最新稳定 Release 提醒更新。
