...
- 图表生成器扩充至 37 类离线模板：22 类 Mermaid 结构图与 15 类 ECharts 数据图表，支持源码编辑、实时预览及 Word／HTML／PDF 导出。
- 新增 15 类数据图表，包括柱状、折线、散点、组合、热力、箱线、气泡、仪表盘、环形、漏斗、瀑布和词云等常用图表。
- macOS 使用原生 Security-Scoped Bookmark 记忆文档与文件夹授权，应用重启后可从最近阅读、收藏和资源浏览器静默恢复访问。
- 修复多图文档预览跳动、定位偏移、C4／ER／状态图标签重叠，以及图表导出不一致等问题；新增中英文真实浏览器巡检覆盖全部 37 类模板。
- 支持参数填写、行内／块级／编号三种插入方式、实时预览，以及直接编辑生成的 Markdown 源码；编号公式会将编号稳定显示在最右侧。
- 新增 Typora 风格 LaTeX、mhchem 化学公式和 `\tag{…}` 公式编号渲染，预览、源码定位、PDF 打印与 Word 导出保持一致。
- 修复 WPS／Word 与独立 HTML 导出在原生公式后重复显示 LaTeX 源码的问题；两种导出现在只保留一份与软件预览一致的公式，HTML 同时保留主题、代码、表格和图片。
- 编辑器内可直接打开官网中英文公式教程，查看可复制的数学、物理和化学示例。
- 优化已移动、删除或被聊天软件清理的最近文档处理，不再重复产生无意义的错误日志；意见反馈详情增加受控的 IP 与城市信息展示。
...
- 编辑工具栏覆盖 H1–H6、加粗、斜体、删除线、高亮、文字颜色、链接、行内/块级代码、引用、列表、任务、分隔线、表格和图片；文字颜色按钮紧跟高亮，提供默认色、7 档灰阶和 40 个综合色阶组成的 48 色完整方格色板，可实时预览、再次换色、恢复默认颜色并完整撤回。空间不足时工具栏自动把按钮收进“更多格式”，不再出现横向滚动条。“更多格式”还可插入粗斜体、下划线、上下标、学科公式、37 类图表（22 类 Mermaid + 15 类数据图表）、强制换行、脚注、引用式链接、自动链接、转义符号、HTML/折叠区块、键盘按键和注释。常用格式支持 `Ctrl/Cmd + B`、`Ctrl/Cmd + I`、`Ctrl/Cmd + K`、`Ctrl/Cmd + Shift + X`、`Ctrl/Cmd + Shift + H`。
...
- 支持 Typora 风格 Mermaid 图表：在 ` ```mermaid ` 围栏代码块中编写 Mermaid 源码，即可实时显示流程图、时序图、甘特图等图表。编辑器“更多格式 → 图表生成器”按用途提供 22 类常用模板，可查看说明、编辑源码、实时预览并一键插入。图表适配明暗模式，语法错误只在对应图表位置提示；Word／HTML 导出会嵌入清晰图片，PDF 使用系统打印保持预览排版。[查看 22 类 Mermaid 图表完整案例](docs/Mermaid-图表完整案例.md)。
...
- 支持导出 Word、HTML 和 PDF：Go 在本地生成标准 DOCX，LaTeX 与 mhchem 学科公式会转换为可缩放、可编辑的 Word 原生公式；HTML 导出为保留当前明暗模式、主题色、公式、代码和图片的独立安全网页；PDF 使用 Windows WebView2／macOS 系统打印面板保持预览样式。
- 内置意见反馈：支持功能建议／功能异常分类、可选联系方式、最多 5 张截图、自动软件与系统版本信息；管理员可在官网后台查看、标记已解决或连同图片一起删除。
- 右侧目录采用可折叠层级树，支持章节定位、当前章节跟随并按文档记忆折叠状态；目录字号和默认宽度会按 1080p／2K／4K 显示器连续适配，仍可通过分隔条手动调整宽度。同时支持文档搜索、打印和回到顶部。
